...
namespace WordMasterApp.Components.LoadingStripe
{
    public class LoadingStripeDrawable : IDrawable
    {
        ...
        public void UpdateAnimations(IList<Color> stripeColors, double speed)
        {
            _animations.Clear();

            if (stripeColors.Any())
            {
                foreach (var color in stripeColors)
                {
                    _animations.Add(new StripeAnimation(color, speed));
                }
            }
        }
...
internal class StripeAnimation
{
    ...
    private readonly float _pulseAngleStep = Rnd(0.05f, 0.15f);
    private readonly float _pulseFactorSmooth = Rnd(0.2f, 0.8f);
    private readonly float _startOffset = Rnd(0.2f, 0.8f);
    private readonly int _blurLayers = 5;

    private ICanvas _canvas;
    private RectF _area;
    private Color _color;
    private float _stepOffset = 2;
    ...
    public StripeAnimation(Color color, double stepOffset)
    {
        
        _color = color;
        _stepOffset = (float)stepOffset * Rnd(0.5f, 2f);
    }
            ...
            for (var x = CurrentOffset - _area.Width - blurShift; x < _area.Width + blurShift; x += _area.Width)
            {
                var gradient = new LinearGradientPaint([
                    new PaintGradientStop(0.3f,  Colors.Transparent),
                        new PaintGradientStop(0.5f, _color.WithAlpha(alphaShift * PulseFactor)),
                ...
                var rect = new RectF(
                    x - blurShift / 2 + padding,  // сдвигаем в рабочую область
                    -blurShift / 2 + padding,
